...
export function Factorial(n, n1) //
{
  const m = n1 || 1;
  if (n <= 0) return (1.0);
  let F = 1.0;
...
export function Yr(l, m, theta) {
  return Math.pow(-1.0, m) * Math.sqrt(l + 1.0 / 2.0) * Factorial(l - m) / Factorial(l + m) * Legendre(m, l, Math.cos(theta));
}